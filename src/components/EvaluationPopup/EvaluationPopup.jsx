  ...
  return (
    <Popup isOpen={isOpen} setIsOpen={setIsOpen} style={{ minWidth: 1000, minHeight: 400 }}>
      ...
      <ul className={styles.content}>
        <li className={styles.section}>
          ...
          <p className={styles.sectionItem}>
            ...
            <span className={styles.itemValue}>{orderData.filename}</span>
          </p>
          <p className={styles.sectionItem}>
            <span className={styles.itemName}>На сервере</span>
            <span className={styles.itemValue}>{orderData.savedFilename}</span>
          </p>
          <p className={styles.sectionItem}>
            <span className={styles.itemName}>Отправлен</span>
            <span className={styles.itemValue}>{getPrettyDateTime(orderData.createdAt)}</span>
          </p>
          <p className={styles.sectionItem}>
            <span className={styles.itemName}>Комментарий</span>
            <span className={styles.itemValue}>{orderData.userComment || '-'}</span>
          </p>
        </li>
        ...
        <li className={styles.section}>
          ...
          <p className={styles.sectionItem}>
            ...
            <span className={styles.itemValue}>{orderData.name}</span>
          </p>
          <p className={styles.sectionItem}>
            <span className={styles.itemName}>Email</span>
            <span className={styles.itemValue}>{orderData.login}</span>
          </p>
        </li>
        ...
        <li className={styles.section} style={{ backgroundColor: 'white' }}>
          ...
          <div className={styles.sectionItem}>
            ...
            <input
              checked={isEvaluated || false}
              onChange={handleIsEvaluatedClick}
              type="checkbox"
              name="isEvaluated"
              id="isEvaluated"
            />
      ...
      <div className={styles.bottom}>
        ...
        <Button onClick={handleSaveButtonClick} buttonDisabled={!isDataChanged}>
          Сохранить
        </Button>
        <Button onClick={handleDownloadButtonClick}>Скачать исходный файл</Button>
        <ButtonAccent
          buttonDisabled={isDataChanged}
          href={`${SITE_URL}:${SITE_PORT}/evaluate?evaluateId=${orderData.orderId}`}
        >
          Выполнить расчет
        </ButtonAccent>
      </div>
