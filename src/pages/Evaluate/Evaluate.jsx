...
const Evaluate = () => {
  ...
  function evaluateButtonClick() {
    ...
    formDataObject.foundGlobally = Object.fromEntries(
      Object.entries(globalSearchSelectedBulls).filter(([, bullId]) => Boolean(bullId)),
    );
      ...
      <section className="evaluate-container">
        <div className="text-block">
          ...
          <Paragraph style={{ maxWidth: 600 }}>
            Ниже можно загрузить таблицу с данными предков и расчитать прогнозируемые показатели потомства
          </Paragraph>
        </div>
      ...
      <section className={`evaluate-container animated ${currentStep.step >= 2 ? '' : 'hidden'}`}>
        <div className="text-block">
          ...
          <Paragraph style={{ maxWidth: 800 }}>
            Теперь необходимо указать, в каких колонках таблицы расположены данные о предках, такие как кличка, семенной
            код, идентификационный или инвентарный номер.
          </Paragraph>
        ...
        <ButtonWithStatus
          ...
          buttonDisabled={
            ...
            (secondStepStatus.status === '' || secondStepStatus.status === 'Error' ? false : true)
          }
        ...
        <div className="text-block">
          ...
          {noMatchesBullsMarkers.length && (
            <>
              ...
              >
                Животные ниже не были найдены в базе по представленным в таблице данным. Но их можно найти в глобальном
                поиске вручную.
              </Paragraph>
