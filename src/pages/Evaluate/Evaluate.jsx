...
const Evaluate = () => {
  ...
  const [secondStepStatus, setSecondStepStatus] = useState({
    visible: false,
    status: "",
    message: "",
  });
  const [thirdStepStatus, setThirdStepStatus] = useState({
    visible: false,
    status: "",
    message: "",
  });
      ...
      .then((data) => {
        ...
        if (parentsWithExtraMatches?.length && parentsWithNoMatches.length) {
          setThirdStepText({
            ...
            paragraph: `Некоторых предков невозможно однозначно идентифицировать по имеющимся данным. Поэтому ниже для каждого такого предка нужно выбрать верного, найдя его по кличке. Также в базе не было найдено ${
              parentsWithNoMatches.length
            } быков: ${parentsWithNoMatches.map((el) => el.name).join(", ")}.`,
          });
          ...
          setThirdStepText({
            ...
            paragraph: `В базе данных не было найдено ${
              parentsWithNoMatches.length
            } быков: ${parentsWithNoMatches.map((el) => el.name).join(", ")}.`,
          });
        } else {
  ...
  function evaluateButtonClick() {
    ...
    formDataObject.foundGlobally = Object.fromEntries(
      Object.entries(globalSearchSelectedBulls).filter(([, bullId]) =>
        Boolean(bullId)
      )
    );
  ...
  function handleInputChange(inputs) {
    if (
      inputs.slice(0, 3).every((input) => input.value) &&
      childColumnInputValue
    ) {
      if (
  ...
  function handleGlobalSearchBullClicked(bullName, foundedBullId) {
    setGlobalSearchSelectedBulls((prev) => ({
      ...prev,
      [bullName]: foundedBullId,
    }));
  }
  ...
  useEffect(() => {
    ...
    if (params.get("evaluateId")) {
      ...
      setUploadFileStatus({
        visible: true,
        message: "Файл уже загружен пользователем",
        status: "Success",
      });

      fetch(
        `${SERVER_URL}:${SERVER_PORT}/api/orders/${params.get(
          "evaluateId"
        )}/table-dump`,
        {
          headers: {
  ...
  return (
    <>
      <section className="evaluate-container">
        <div className="text-block">
          ...
          <Paragraph style={{ maxWidth: 600 }}>
            Ниже можно загрузить таблицу с данными предков и расчитать
            прогнозируемые показатели потомства
          </Paragraph>
        </div>
      ...
      <section
        className={`evaluate-container animated ${
          currentStep.step >= 2 ? "" : "hidden"
        }`}
      >
        <div className="text-block">
          <SectionHeading style={{ marginBottom: 10 }}>
          ...
          <Paragraph style={{ maxWidth: 800 }}>
            Теперь необходимо указать, в каких колонках таблицы расположены
            данные о предках, такие как кличка, семенной код, идентификационный
            или инвентарный номер.
          </Paragraph>
          <Paragraph style={{ maxWidth: 700 }}>
            При отсутствии каки-либо показателей поля нужно оставить пустыми.
          </Paragraph>
          <Paragraph style={{ maxWidth: 700 }}>
            Для удобства ниже находятся первые строки загруженной таблицы.
          </Paragraph>

          <PreviewTable tableDump={tableDump} style={{ marginTop: 50 }} />

          <div className="child-column-picker" style={{ marginTop: 50 }}>
            <div className="child-column-picker__title">
              Столбец с кличкой потомка
            </div>
            <input
              type="text"
              name="child-column"
        ...
        <ButtonWithStatus
          ...
          buttonDisabled={
            ...
            (secondStepStatus.status === "" ||
            secondStepStatus.status === "Error"
              ? false
              : true)
          }
      ...
      <section
        className={`evaluate-container animated ${
          currentStep.step >= 3 ? "" : "hidden"
        }`}
      >
        <div className="text-block">
          <SectionHeading style={{ marginBottom: 10 }}>
            3. {thirdStepText.title}
          </SectionHeading>
          <Paragraph style={{ maxWidth: 800, lineHeight: 1.5 }}>
            {thirdStepText.paragraph}
          </Paragraph>
          ...
          {noMatchesBullsMarkers.length && (
            <>
              ...
              >
                Животные ниже не были найдены в базе по представленным в таблице
                данным. Но их можно найти в глобальном поиске вручную.
              </Paragraph>
        ...
        <ButtonWithStatus
          ...
          buttonDisabled={
            thirdStepStatus.status === "" || thirdStepStatus.status === "Error"
              ? false
              : true
          }
